...
export const parseParams = (formValues: FilterFormData): FindAllParams => {
  ...
  if (formValues.price) {
    parsedData.price = [
      ...
      formValues.price.max !== undefined ? formValues.price.max : undefined,
    ];
  }

  if (formValues.float) {
    parsedData.float = [
      formValues.float.min !== undefined ? formValues.float.min : 0,
      formValues.float.max !== undefined ? formValues.float.max : undefined,
    ];
  }
